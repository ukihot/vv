...
import Application.User.Boundary.Input.ActivateUserUseCase (ActivateUserUseCase (execute))
...
import Domain.User.ValueObjects.UserId (mkUserId, unUserId)
...
runActivateUser st rawUserId = do
  (_, nextState) <-
    runStateT
      (unBackendM (execute (ActivateUserRequest rawUserId)))
      st
  pure nextState
