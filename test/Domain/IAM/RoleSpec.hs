...
case_rolePermissionWorkflow = do
  ...
  assertEqual "role granted profile" (RoleProfile roleName [permissionId]) (getRoleProfile grantedRole)
  ...
  assertEqual "role revoked event" (PermissionRevokedFromRole actorId roleId permissionId) revokedEvent
