...
本システムはネットワークに依存しない。
Write 側に **SQLite**（イベントストア）、Read 側に **acid-state**（インメモリ・マテリアライズドビュー）を採用し、すべてプロセス内で完結する。
サーバもコンテナも不要。実行バイナリとデータファイルだけで動く。

Write API が SQLite にイベントを書き込むと、Projector が acid-state の Read モデルにイベントを適用する。
クエリは acid-state 上のインメモリ Haskell データ構造を直接参照するため、レイテンシがほぼゼロになる。
...
SQLite は単一ファイルでトランザクション ACID を保証し、イベントの append-only ストアとして機能する。
再起動時は SQLite のイベント列を acid-state にリプレイして Read モデルを再構築する。
acid-state は `Data.Map` や `Data.IntMap` などの Haskell ネイティブなデータ構造をそのまま永続化でき、
複雑なネスト構造や計算済みの集計値も型安全に保持できる。
...
本システムでは **Write 側に SQLite（イベントストア）、Read 側に acid-state（インメモリ・マテリアライズドビュー）** を使う。
SQLite はイベントの append-only ストアとして ACID を保証する。
acid-state は Haskell ネイティブなデータ構造（`Data.Map` 等）をそのままインメモリで保持し、
クエリは純粋な Haskell 関数として記述できる。SQL もシリアライズも不要で、レイテンシがほぼゼロになる。
どちらもプロセス内で直接操作できる組み込み型であり、外部サービスへの依存がない。

Write API が SQLite にイベントを書き込んだ後、Projector が
型付きイベントを acid-state に適用して Read モデルを更新する。
...
純粋な畳み込み関数で acid-state の状態を更新する。
GHC ランタイムが数万スレッドを OS スレッド数本で多重化するので、
Projector を集約単位・ビュー単位で並列化してもオーバーヘッドは小さい。

再起動時は SQLite の最終シーケンス番号と acid-state が保持する最終反映済み番号を比較し、
差分のイベントを SQLite から読み直して acid-state に再適用（リプレイ）することで整合性を回復する。
...
Infrastructure : SQLite (EventStore)  acid-state (Read Store / インメモリ)
...
- **Read 側の acid-state** はインメモリの Haskell データ構造をそのまま保持するマテリアライズドビュー。Projection が非同期で更新する。
- **Projection は Infrastructure 層にいる。** SQLite からイベントを読み、純粋関数で acid-state の状態を更新する。DTO は作らない。
- **非同期 Projector** は `forkIO` + `TBQueue` で実装する。Write のコミット完了後にイベント通知を送り、Projector スレッドが acid-state を更新する。Eventually Consistent。
- **QueryService の Port（型シグネチャ）は Application 層に定義する。** 実装は Infrastructure 層で acid-state を参照する純粋な Haskell 関数。返り値は内部 DTO。
...
| 68 | Read の整合性レベル | Read が常に最新であることを前提にすると、設計が壊れる。 | Read（acid-state）は Eventually Consistent であることを明示する。最新性が必要な場合は Write 側（SQLite）に問い合わせる。 |
| 69 | Projection 更新戦略 | 同期か非同期かが未定義だと、レイテンシとスループットのトレードオフが読めない。 | 本システムでは非同期を採用する。SQLite への書き込み完了後、`forkIO` + `TBQueue` で Projector スレッドに通知し、acid-state を更新する。Haskell の軽量スレッドと STM により、外部メッセージブローカーなしで安全に非同期化できる。再起動時は SQLite からのリプレイで整合性を回復する。 |
...
さらに、GHC の軽量スレッドと STM は、Write（SQLite）から Read（acid-state）への
非同期 Projection パイプラインを、外部インフラなしで安全に実現する。
これらは、壊れると致命的な会計領域で事故を構造的に防ぐ仕組みであり、
他言語の型システムでは同等の保証を得られない。

**第三に、インフラの最小化。**
SQLite と acid-state はどちらもプロセス内で完結する組み込み型である。
